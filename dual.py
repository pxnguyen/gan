...
class Dual(object):
  def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
         ...
         gfc_dim=1024, dfc_dim=1024, c_dim=3, dataset_name='default', norm='l1',
  ...
  def build_model(self):
    ...
    #fc1f = tf.reduce_mean(fc1f, axis=0)
    #self.h3_fake = tf.reshape(h3_fake, [1, -1])
    ...
    differences = alpha * tf.reshape(fake_data - real_data, [self.batch_size, -1])
    differences = tf.reshape(differences, tf.shape(fake_data))
    interpolates = real_data + differences
    h1, h2, fc1, fc2 =\
      self.discriminator(interpolates, self.y, reuse=True)
    gradients = tf.gradients(h2, [interpolates])[0]
    gradients = tf.reshape(gradients, [self.batch_size, -1])
    slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), reduction_indices=[1]))
    gradient_penalty = tf.reduce_mean((slopes-1.)**2)
    self.gp_cost = 10*gradient_penalty

    diff = layer_real - layer_fake
    if self.norm == 'l1':
      l2_loss = tf.sqrt(tf.reduce_sum(tf.abs(diff)))
    elif self.norm == 'l2':
      l2_loss = tf.sqrt(tf.reduce_sum(tf.square(diff)))

    category_loss = self.category_loss_real + self.category_loss_fake
    if self.version == 'dual-gp':
        self.d_loss = -l2_loss + self.gp_cost
    elif self.version == 'dual-clip':
        self.d_loss = -l2_loss
    self.g_loss = l2_loss
    self.d_loss += category_loss
    self.g_loss += self.category_loss_fake
    self.gan_loss = self.d_loss

    self.l2_loss_sum = scalar_summary("l2_loss", l2_loss)
    ...
    self.d_all_vars = self.d_vars + self.dfc_vars
  ...
  def train(self, config):
    ...
    #d_optim = tf.train.AdamOptimizer(1e-4, beta1=config.beta1, beta2=0.9) \
    #          .minimize(self.d_loss, var_list=self.d_vars)
    #g_optim = tf.train.AdamOptimizer(1e-4, beta1=config.beta1, beta2=0.9) \
    #          .minimize(self.g_loss, var_list=self.g_vars)
    try:
    ...
    self.d_sum = merge_summary([self.d_loss_sum, self.l2_loss_sum,
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, batch_idxs):
        ...
        errD, errG, errGP, errC_real, errC_fake = self.sess.run([
          self.gan_loss, self.g_loss, self.gp_cost,
          self.category_loss_real, self.category_loss_fake],
                feed_dict=data_dict)
        print 'epoch %d iter %d d_cost %0.7f g_cost %0.7f, gp_cost %0.7f, c_real %0.7f, c_fake %0.7f'\
              % (epoch, idx, errD, errG, errGP, errC_real, errC_fake)

        counter += 1

        # writing to summaries
