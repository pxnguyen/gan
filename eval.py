...
class DCGAN(object):
  ...
  def build_model(self):
    ...
    self.D, self.D_logits, self.real_cat_logits = self.discriminator(inputs, self.y, reuse=False)

    self.sampler = self.sampler(self.z, self.y)
    self.D_, self.D_logits_, self.fake_cat_logits = self.discriminator(self.G, self.y, reuse=True)
  ...
  def eval(self, config):
    ...
    batch_idxs = min(len(data_X), config.train_size) // config.batch_size
    _, _, cat_scores = self.discriminator(self.inputs, reuse=True)
